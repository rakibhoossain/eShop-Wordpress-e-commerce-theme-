...
class Eshop_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			// 'home_prefix'            => get_theme_mod( 'eshop_blog_breadcrumb_menu_prefix', __( 'You Are Here', 'eshop' ) ),
			// 'separator'              => get_theme_mod( 'eshop_blog_breadcrumb_menu_separator', 'rarr' ),
			'show_post_type_archive' => '1',
			'show_terms'             => get_theme_mod( 'eshop_blog_breadcrumb_menu_post_category', 1 ),
			'home_label'             => esc_html__( 'Home', 'eshop' ),
			'tag_archive_prefix'     => esc_html__( 'Tag:', 'eshop' ),
			'search_prefix'          => esc_html__( 'Search:', 'eshop' ),
			'error_prefix'           => esc_html__( '404 - Page not Found', 'eshop' ),
		);

		// Setup a filter for changeable variables and meger it with the defaults
		$args     = apply_filters( 'eshop_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<ul class="b-crumbs">%s</ul>', $this->html_markup );
	}
	...
	// 		if ( $this->home_prefix ) {
	// 			$prefix = sprintf( '<span class="eshop-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
	// 		}
	// 	}
	...
	private function get_breadcrumb_leaf_markup( $object_type = '' ) {
		...
		return sprintf( '<li><span>%s</span></li>', $title );
	}
	 ...
	 */
	private function get_single_breadcrumb_markup( $title, $link = '') {
		...
		$breadcrumb_content = sprintf( '%s', $title );

		// If a link is set add its markup
		if ( $link ) {
			$breadcrumb_content = sprintf( '<a href="%s" >%s</a>', $link, $breadcrumb_content );
		}

		// If a separator should be added, do it
		// if ( $separator ) {
		// 	$separator_markup = sprintf( '<span class="eshop-breadcrumb-sep">%s</span>', $this->separator );
		// }

		return sprintf( '<li>%s</li>', $breadcrumb_content);
